...
public class MainWindow extends JFrame
{
	...
	private KeyboardWindow keyboard=new KeyboardWindow();//键盘框框
						...
						for(Monster tempgw:gw[mapNo-1])
						{
							if(!(attacking==1&&(Direction==0?((tempgw.getx()<=map.getrenx()&&tempgw.getx()>=map.getrenx()-115)?1:0):((tempgw.getx()>=map.getrenx()&&tempgw.getx()<=map.getrenx()+115)?1:0))==1&&(tempgw.gety()<=map.getreny()&&tempgw.gety()>=map.getreny()-100)))
							{
								if(tempgw.getTarget()==0)
								{
									...
									case 3:
										{for(int i=0;i<10;i++)
											{if(tempgw.getx()>=tempgw.getxmin()) tempgw.movex(-2); if(tempgw.gety()==map.getreny()&&tempgw.getx()>=map.getrenx()-10&&tempgw.getx()<=map.getrenx()+10){new Thread(Battack).start();badirection=1;}tempgw.setzhuangtai(2); MainWindow.this.repaint();}
										break;
										}
									case 4:
									case 5:
										{for(int i=0;i<10;i++)
											{if(tempgw.getx()<=tempgw.getxmax()) tempgw.movex(2);if(tempgw.gety()==map.getreny()&&tempgw.getx()>=map.getrenx()-10&&tempgw.getx()<=map.getrenx()+10){new Thread(Battack).start();badirection=0;} tempgw.setzhuangtai(3); MainWindow.this.repaint();}
										break;
										}
									}
